...
db_exp_total <- db %>% 
  ...
  distinct() %>%
  as.data.frame() %>%
  add_row(exp_route = "eggs", dose_a = 0, dose_sd_a = NA, nExp = 7, nInf = 0) %>%
  add_row(exp_route = "proglottids", dose_a = 0, dose_sd_a = NA, nExp = 3, nInf = 0) %>%
...
db_exp_brain <- db[!is.na(db$brain_cyst),] %>% 
  ...
  distinct() %>%
  as.data.frame() %>%
  add_row(exp_route = "eggs", dose_b = 0, dose_sd_b = NA, nExp = 7, nInf = 0) %>%
  add_row(exp_route = "proglottids", dose_b = 0, dose_sd_b = NA, nExp = 3, nInf = 0) %>%
...
db_oral_total <- db_oral_total[,c(6,2:5)] %>%
  add_row(exp_route_b = "oral", dose_a = 0, dose_sd_a = NA, nExp = 7, nInf = 0)
...
db_oral_brain <- db_oral_brain[,c(6,2:5)] %>%
  add_row(exp_route_b = "oral", dose_b = 0, dose_sd_b = NA, nExp = 7, nInf = 0)
...
geom_total$cyst_type <- "total"
geom_ves$cyst_type <- "vesicular"
geom_brain$cyst_type <- "brain"
...
geom_oral <- rbind(geom_total[geom_total$exp_route!="carotid",], 
                   geom_ves[geom_ves$exp_route!="carotid",],
                   geom_brain[geom_brain$exp_route!="carotid",])

geom_oral$cyst_type <- factor(geom_oral$cyst_type, 
                              levels = c("total","vesicular","brain"))



geom_egg <- rbind(geom_total[geom_total$exp_route=="eggs",], 
                  geom_ves[geom_ves$exp_route=="eggs",],
                  geom_brain[geom_brain$exp_route=="eggs",])

geom_egg$cyst_type <- factor(geom_egg$cyst_type, 
                             levels = c("total","vesicular","brain"))



geom_prog <- rbind(geom_total[geom_total$exp_route=="proglottids",], 
                  geom_ves[geom_ves$exp_route=="proglottids",],
                  geom_brain[geom_brain$exp_route=="proglottids",])

geom_prog$cyst_type <- factor(geom_prog$cyst_type, 
                             levels = c("total","vesicular","brain"))



geom_bee <- rbind(geom_total[geom_total$exp_route=="beetles",], 
                  geom_ves[geom_ves$exp_route=="beetles",],
                  geom_brain[geom_brain$exp_route=="beetles",])

geom_bee$cyst_type <- factor(geom_bee$cyst_type, 
                             levels = c("total","vesicular","brain"))



geom_car <- rbind(geom_total[geom_total$exp_route=="carotid",], 
                  geom_ves[geom_ves$exp_route=="carotid",],
                  geom_brain[geom_brain$exp_route=="carotid",])

geom_car$cyst_type <- factor(geom_car$cyst_type, 
                             levels = c("total","vesicular","brain"))
